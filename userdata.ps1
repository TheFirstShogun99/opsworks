<powershell>
 ...
 $env:CHEF_SERVER_NAME = "my-chef-automate" # Name of your Chef Server
 $env:CHEF_SERVER_ENDPOINT = "my-chef-automate-uv2qyhrahfdyyebp.ap-southeast-1.opsworks-cm.io" # FQDN of your Chef Server
 ...
 $env:EC2_TOKEN = "$(Invoke-RestMethod -Headers @{"X-aws-ec2-metadata-token-ttl-seconds" = "21600"} -Method PUT –Uri http://169.254.169.254/latest/api/token)"
 $env:CHEF_NODE_NAME = "$(Invoke-RestMethod -Headers @{"X-aws-ec2-metadata-token" = $token} -Method GET -Uri http://169.254.169.254/latest/meta-data/instance-id)" # Use EC2 Instance ID as Chef Node Name

 # Optional environment variables
 $env:ROOT_CA_URLS = @("https://opsworks-cm-$env:REGION-prod-default-assets.s3.amazonaws.com/misc/opsworks-cm-ca-2020-root.pem") # URL to download the Root CA certificate of your Chef Automate server. Change this only if you are using a server with a custom domain.
...
"@
 ...
 function install_trusted_certs {
     ...
     if ("$env:ROOT_CA_URLS") {
        Push-Location
        cd $env:CHEF_TRUSTED_CERTS_PATH
        For ($i=0; $i -le $env:ROOT_CA_URLS.length; $i++) {
            $url = $env:ROOT_CA_URLS[$i]
            $file_name = $url.Split("/")[-1]
            $download_description = "root ca cert"
            download_with_retries "$url" "$file_name" $download_description
        }
        Pop-Location
     }
 }
